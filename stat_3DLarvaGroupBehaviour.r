...
library(rjags) ## MCMC Sampler
library( rgl ) ##For 3D rendering Plot
...
################
## Params for model run
steps <-15000 ## Number of per chain before stop the MCMC - this number is enough for convergence
nchains <- 7
nthin <- 5
...
#save(draw_NF,draw_LF,draw_DF,file = paste0("stat_Larval3DGaussianBehaviouModel_RJags.RData"))
...
###################################################
  ...
  ntail <- 130 ##How many samples (spheres) to show for each model
